...
- Reaps and then wipes its dir before each launch. Using the app cache instead of shared system temp avoids macOS “access data from other apps” prompts; wiping removes stale `SingletonLock` files after unclean shutdowns. **Wiping alone does not stop a leftover Chromium from capturing the launch** — on Windows the process singleton's marker is a hidden message window named after the profile path, held in the leftover's own memory, so deleting the directory leaves the handoff armed (and Windows fails the delete outright while that process holds the profile open). The owner has to be killed, which is why `reap_profile` runs on the dir immediately before every launch, ahead of the wipe (see Lifecycle and cleanup).
...
Only the *successor* process can clean that up. [src-tauri/src/leftover_browsers.rs](src-tauri/src/leftover_browsers.rs) `reap` kills every process still holding one of this app's profile dirs. `setup()` sweeps all three at startup, and `reap_profile` in `lib.rs` reaps the single target dir before every launch — sessions are given up and relaunched throughout a run, and a leftover holding a dir *captures* the next launch into it rather than merely wasting memory.
...
- **The per-launch reap is safe for the same reason, differently argued.** It targets one dir, and each dir belongs to exactly one session; every caller reaches `launch_browser` holding that session's lock with its state already emptied, so no browser this run still wants is running out of that dir.
- **A launch that fails with exit status 0 and empty stderr is a handoff that beat the snapshot**, not a crash — Chromium exits before logging, while a crash, a rejected flag or an unopenable profile all carry a non-zero status. `launch_browser` detects that through `is_singleton_handoff` and retries `spawn_browser` **once**; the retry's value is the second reap, not the second launch. Do not widen it to other failures or to more attempts — a second handoff is not a race, and an unexplained silent exit has to reach the user.
